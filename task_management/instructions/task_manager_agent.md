...
- [L] Task name - Strategic decision that shapes direction
- [N] Task name - Routine execution, good enough quality
- [O] Task name - Administrative checkbox work

**On Deck** (Status = "On Deck")
- [Next up after current work with LNO classifications]

**This Week - Due Soon** (due in next 3 days)
- [Tasks sorted by due date with LNO classifications]

**This Week - By Project**
- [Group remaining This Week tasks by Project with LNO classifications]

**Overdue** (due date < 2026-01-11)
- [Flag prominently with how many days overdue and LNO classifications]

**LNO Summary**: L: X (focus deeply), N: Y (good enough), O: Z (minimize time)

**Recommendation**: [Your intelligent suggestion based on urgency + project priority + blockers + LNO]
```
...
**LNO Framework Integration:**
- Import and use `classify_task_lno()` from `tools.task_manager_agent.classify_task_lno` for each task
- Call `get_rajiv_context()` to get strategic context and pass to `classify_task_lno()` as `strategic_context`
- For each task, show LNO classification: `[L/N/O] Task name - [brief reason from classification.reasoning]`
- Group tasks by LNO classification in addition to existing grouping (optional: show L tasks first, then N, then O)
- Add LNO summary at the end: "L tasks: X (focus deeply), N tasks: Y (good enough), O tasks: Z (minimize time)"
- When suggesting priorities, consider LNO: "Focus deeply on [L task] first, then handle [N tasks] efficiently, minimize time on [O tasks]"
- LNO classifications are suggestions - present them helpfully but don't be prescriptive
...
**Flow**:
1. Present all inbox tasks with suggested project, status, due date, and LNO classification
...
**LNO Integration:**
- For each inbox task, use `classify_task_lno()` to suggest L/N/O classification
- Include LNO in the presentation: `[L/N/O] Task name`
- This helps user understand effort investment when triaging
...
1. [L] 'Review Q4 plan with Reid'
   → Reporting Pod project, This Week, due Jan 17
   → High leverage: Strategic planning work
   
2. [N] 'Watch autonomous agents video'  
   → No project, Backlog (learning resource)
   → Neutral: Routine learning, good enough quality
   
3. [N] 'Email Paolo about hiring'
   → PM Hiring project, Top Priority, due today
   → Neutral: Routine communication
   
4. [O] 'Schedule EPD meeting'
   → Overdue (was due Jan 8), This Week
   → Overhead: Administrative scheduling
...
- LNO Distribution: L: [X] (focus deeply), N: [Y] (good enough), O: [Z] (minimize time)
...
- ⚠️ Too many L tasks ([X]) - may indicate over-investment in high-leverage work
- ⚠️ Too many O tasks ([X]) - consider delegating or automating
...
- **LNO Analysis**: Classify all This Week tasks using `classify_task_lno()` and show distribution
- Flag if too many L tasks (may indicate over-investment) or too many O tasks (consider delegating)
...
5. **LNO classification** (L tasks deserve deep focus, N tasks good enough, O tasks minimize time)
6. **Age** (very old tasks might be stale)
...
- "Focus deeply on [L task] - high leverage work. Handle [N tasks] efficiently. Minimize time on [O tasks]."
